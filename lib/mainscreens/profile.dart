...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<DealData> getDealOfTheDay() async {
    ...
    List<Location> latLongFromAddress = await LocationService().getLatLongFromAddress(deal.address!);
    ...
    setState(() {
      distance = dist < 99 ? (dist < 1 ? "<1 Mile" : "${dist.toStringAsFixed(0)} Miles")
          : "99+ Miles";
    });
    return deal;
  }
  ...
  Widget build(BuildContext context) {
    ...
    List<String> accounts = ["Contact Us", "Preferences", "Deals History", "Account\nInformation", "Notifications",];
    ...
    return GestureDetector(
      onTap: () {
        if(options == 0) {
          ...
          Feedback.forTap(context);
          Share.share("Come check out Myrtle Beach Passport!");
        }
      },
      child: Stack(
        children: [
