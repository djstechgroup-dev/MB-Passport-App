...
class BusinessApi {
  ...
  Future<void> getAllBusiness(String? token) async {
    try {
      //Temporary 10.0.2.2:8000 for testing using emulator
      final uri = Uri.http('10.0.2.2:8000', '/api/business');
      final headers = {
      ...
      if(response.statusCode == 200) {
        print(output);
        print(output['businesses'][0]['businessName']);
        return output['businesses'][0]['businessName'];
      } else {
        throw Exception("ERROR WITH STATUS: ${response.statusCode}");
      }
    } catch(e) {
